...
/**
 * Frontmatter for the fixed schema below: `key: value` pairs, plus inline
 * `[a, b]` lists. Not a general YAML parser, and not trying to be — the
 * fields are validated right after, so a malformed post fails the build
 * loudly instead of publishing something wrong.
 */
...
function parse(file: string): Post {
  ...
  const { data, content } = frontmatter(readFileSync(join(POSTS_DIR, file), 'utf8'))
